...
#         st = torch.load(os.path.join(os.environ['HOME'], '.cache', 'moby_swin_t_300ep_pretrained.pth'), map_location='cpu')
        ...
        weight_path = os.path.join(weights_dir, 'moby_swin_t_300ep_pretrained.pth')
        if not os.path.exists(weight_path):
            st = gdown.download("https://drive.google.com/u/0/uc?id=1PS1Q0tAnUfBWLRPxh9iUrinAxeq7Y--u&export=download", 
                               weight_path, quiet=False)
...
class CLIP(torch.nn.Module):
    ...
    def __init__(self, cv_type='adv'):
        ...
        weights_dir = os.path.join(os.getcwd(), 'weights')
        os.makedirs(weights_dir, exist_ok=True)
        weight_path = os.path.join(weights_dir, 'ViT-B-32.pt')
        self.model, _ = clip.load(weight_path, jit=False, device='cpu')
#         self.model, _ = clip.load("./weights/ViT-B-32.pt", jit=False, device='cpu')
