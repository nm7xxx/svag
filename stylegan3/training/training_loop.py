    ...
    if rank == 0:
        ...
        if wandb_log:
            ...
            try:
                ...
                parent_run_dir = os.path.dirname(os.path.abspath(run_dir))
                cur_tfevents_path = os.path.join(parent_run_dir, 'latest')

                if not os.path.exists(cur_tfevents_path):
                    os.makedirs(cur_tfevents_path)
                else:
                    if os.listdir(cur_tfevents_path):
                        shutil.rmtree(cur_tfevents_path)
                        os.makedirs(cur_tfevents_path)

                stats_tfevents = tensorboard.SummaryWriter(cur_tfevents_path)
    ...
    while True:
        ...
        if stats_tfevents is not None or wandb_log:
            ...
            if wandb_log:
                ...
                for fname in os.listdir(cur_tfevents_path):
                    if fname.startswith('events.out.tfevents'):
                        src = os.path.join(cur_tfevents_path, fname)
